    ...
    $.extend({
        MigrationData: function(options){
            ...
            function storageData(){
                createCookie(1);
                var  storageDataInterval = setInterval(function(){
                    var request_post = getRequestPost();
                    ...
                    callProcess(settings.url, data).done(function(response){
                        ...
                        if(response.status == 'finish'){
                            clearInterval(storageDataInterval);
                            deleteCookie();
            ...
            function processImport(){
                ...
                var importInterval = setInterval(function(){
                    ...
                    callProcess(settings.url, data).done(function(response){
                        ...
                        if(response.data.entities != undefined){
                            var entities = response.data.entities;
                            Object.keys(entities).forEach(function (entity) {
                                var entityData = entities[entity];
                                processBarResult(entity, entityData.total, entityData.import, entityData.error, entityData.point);
                                processBarAnimate(entity, entityData.finish);
                            });
                        }
                    }).fail(function(xhr, status, error){
            ...
            function run(){
                ...
                setTimeout(function(){
                    var current_step = $('#migration-current-step').val();
                    if(current_step == 'storage'){
                        storageData();
                    } else if(current_step == 'import'){
                        processImport();
                    }
                }, 1000);
                ...
                var seo_plugin = $('#seo-plugin-select', container);
                if(seo_plugin.length > 0){
                    var seo_value = seo_plugin.val();
                    if(seo_value){
                ...
                $(container).on('click', '#config-wrap #entity-section input.entity-input-type', function(){
                    ...
                    if(check){
                        _this.parents('li').each(function(index, elm){
                            $(elm).children('div').find('input').prop('checked', true);
                        });
                    } else {
                        var current = _this.closest('li');
                        current.find('ul input').prop('checked', false);
                    }
                });
                ...
                $(container).on('click', '#btn-run-full', function(){
                    $(document).trigger('clickFullMigration');
                });
            }
